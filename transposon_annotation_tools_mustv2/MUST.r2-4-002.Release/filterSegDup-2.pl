...
my%egC2Flag=();
$tempk=0;%tidx=();
for($tempi=0;$tempi<@egCList;$tempi++){@egLList=();
$tline=$egC2IDs{"$egCList[$tempi]"};
$tline=~s/^\;+//g;
$tline=~s/\;+$//g;
@tlist=split(/;/,$tline);
for($tempj=0;$tempj<@tlist;$tempj++){$egLList[$tempj]=$egLength[$tlist[$tempj]];}if(&efFilterLengthVar(@egLList)==1){$egC2Flag{"$egCList[$tempi]"}=1;
$tidx{"$egCList[$tempi]"}=1;$tempk++;}else{$egC2Flag{"$egCList[$tempi]"}=0;}}print" [MinCopyCluster:".(scalar keys%tidx)."]";
$tempk=0;%tidx=();
for($tempi=0;$tempi<@egCList;$tempi++){@egLList=();
$tline=$egC2IDs{"$egCList[$tempi]"};
$tline=~s/^\;+//g;
$tline=~s/\;+$//g;
@tlist=split(/;/,$tline);
for($tempj=0;$tempj<@tlist;$tempj++){$egLList[$tempj]=length($egDR1[$tlist[$tempj]]);}if(&efFilterLengthVar(@egLList)==0){$egC2Flag{"$egCList[$tempi]"}=0;}}for($tempi=0;$tempi<@egCList;$tempi++){if($egC2Flag{"$egCList[$tempi]"}==1){$tempk++;$tidx{"$egCList[$tempi]"}=1;}}print" [MinTSDCopyCluster:".(scalar keys%tidx)."]";
my@egPreSiteList=();
$tempk=0;%tidx=();
for($tempi=0;$tempi<@egCList;$tempi++){@egLList=();
$tline=$egC2IDs{"$egCList[$tempi]"};
...
@tlist=split(/;/,$tline);
@egPreSiteList=();
for($tempj=0;$tempj<@tlist;$tempj++){$egPreSiteList[$tempj]=$egPreSite[$tlist[$tempj]];}if(&efFilterSeqDup(@egPreSiteList)==0){$egC2Flag{"$egCList[$tempi]"}=0;}}for($tempi=0;$tempi<@egCList;$tempi++){if($egC2Flag{"$egCList[$tempi]"}==1){$tempk++;$tidx{"$egCList[$tempi]"}=1;}}print" [NonSeqDupCluster:".(scalar keys%tidx)."]";
print" [done]\n";
...
$tid=$egC2IDs{"$egCList[$tempi]"};
my@egIDList=split(/;/,$tid);
for($tempj=0;$tempj<@egIDList;$tempj++){print efO"$egDataLine[$egIDList[$tempj]]\n";
$nMITE++;}}}close(efO);
print" [done] [MITEs:$nMITE] [Cluster:$nCluster]\n";
sub efFilterSeqDup{my@ql=@_;
open(tO,">temp-MUST-filterSeqDup-2.fasta")or die"EE!\n";
for(my$qi=0;$qi<@ql;$qi++){print tO">$qi\n$ql[$qi]\n";}close(tO);
if(system("formatdb -p F -i temp-MUST-filterSeqDup-2.fasta 2>/dev/null 1>/dev/null")){die"-Error: formatdb!\n";}if(system("megablast -d temp-MUST-filterSeqDup-2.fasta -i temp-MUST-filterSeqDup-2.fasta -o temp-MUST-filterSeqDup-2.self-megablast.txt -e 1e-5 -F F -m 9")){die"-Error: megablast!\n";}open(tI,"temp-MUST-filterSeqDup-2.self-megablast.txt")or die"QQD!\n";
my%qIdentPair=();
while(<tI>){my$qline=$_;$qline=~s/[\r\n]//g;
my@qlist=split(/\t/,$qline);
if($qline=~/^#/){}elsif(@qlist>=11){my($qa,$qb)=($qlist[0],$qlist[1]);my$qc=0;
if($qa>$qb){$qc=$qa;$qa=$qb;$qb=$qc;}my$qident=int($qlist[2]*$qlist[3]/100);
my$qold=$qIdentPair{"$qa|$qb"};
if((defined$qold)and($qold>$qident)){}else{$qIdentPair{"$qa|$qb"}=$qident;}}}close(tI);
my%qPC2ID=();my@qID2PC=();my$egCutOffMinIdent=0.30;
for(my$qi=0;$qi<@ql;$qi++){my$qid=$qi;
my$qMinI=length($ql[$qi])*$egCutOffMinIdent;
for(my$qj=0;$qj<$qi;$qj++){my$qMinJ=length($ql[$qj])*$egCutOffMinIdent;
my$qMaxIdentIJ=$qIdentPair{"$qj|$qi"};
if((defined$qMaxIdentIJ)and(($qMaxIdentIJ>=$qMinI)or($qMaxIdentIJ>=$qMinJ))){$qid=$qj;
last;}}if($qid!=$qi){my$qq=$qID2PC[$qid];
my$qll=$qPC2ID{"$qq"};
$qPC2ID{"$qq"}="$qll;$qi";
$qID2PC[$qi]=$qq;}else{$qPC2ID{"$qi"}="$qi";
$qID2PC[$qi]=$qi;}}my@qqC=keys%qPC2ID;
system("rm -rf \"temp-MUST-filterSeqDup-2\"*");
if(scalar@qqC>=$egMinCopy){return 1;}else{return 0;}}sub efFilterLengthVar{my@ql=@_;
my%qL2ID=();my@qID2L=();
for(my$qi=0;$qi<@ql;$qi++){my$qid=$qi;
for(my$qj=0;$qj<$qi;$qj++){if(abs($ql[$qi]-$ql[$qj])<=$egLengthVar){$qid=$qj;last;}}if($qid!=$qi){my$qq=$qID2L[$qid];
my$qll=$qL2ID{"$qq"};
$qL2ID{"$qq"}="$qll;$qi";
$qID2L[$qi]=$qq;}else{$qL2ID{"$ql[$qi]"}="$qi";
$qID2L[$qi]=$ql[$qi];}}my@qqC=keys%qL2ID;
my$qqMaxCopy=0;
for(my$qi=0;$qi<@qqC;$qi++){my@qqll=split(/;/,$qL2ID{"$qqC[$qi]"});
if(scalar@qqll>$qqMaxCopy){$qqMaxCopy=scalar@qqll;}}if($qqMaxCopy>=$egMinCopy){return 1;}else{return 0;}}sub efSameDR{my($aa,$bb)=@_;
