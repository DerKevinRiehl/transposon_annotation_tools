...
if(system("echo \"$lCmdLine\" > $egDirTemp/getMCL.log")or system("$lCmdLine 2>&1 >>$egDirTemp/getMCL.log")){print"--Error when clustering the MITEs using MCL!\n---[$lCmdLine]---\n";
die"--Error when clustering the MITEs using MCL!\n---[$lCmdLine]---\n";}if($lFileCluster=~/\//){}else{$lFileCluster="$egDirTemp/$lFileCluster";}open(efIn,"$lFileCluster")or die"Error when loading the clustering results!\n--[$lFileCluster]---\n";
%tidx=();
