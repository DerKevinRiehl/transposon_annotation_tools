...
$tline=~s/[\r\n]//g;
$tData=$tData."$tline\n";
if($tline=~/^\/\/$/){$tEScore=$tDR+$tTIR;
$tFlag=1;
for($tempi=$egNum-1;$tempi>=0;$tempi--){if($egEnd[$tempi]<$tStart){last;}if(($egFlag[$tempi]==1)and($tFlag==1)and(&efIsOverlap($egStart[$tempi],$egEnd[$tempi],$tStart,$tEnd)==1)){if($egEScore[$tempi]<$tEScore){$egFlag[$tempi]=1;
$egStart[$tempi]=$tStart;
$egEnd[$tempi]=$tEnd;
$egLength[$tempi]=$tLength;
$egTIR[$tempi]=$tTIR;
$egDR[$tempi]=$tDR;
$egEScore[$tempi]=$tDR+$tTIR;
$egData[$tempi]=$tData;}$tFlag=0;
...
$tempk=0;
for($tempi=0;$tempi<$egNum;$tempi++){if($egFlag[$tempi]==1){for($tempj=$tempi+1;$tempj<$egNum;$tempj++){if(($egFlag[$tempj]==1)and(&efIsOverlap($egStart[$tempi],$egEnd[$tempi],$egStart[$tempj],$egEnd[$tempj])==1)){if($egEScore[$tempi]>=$egEScore[$tempj]){$egFlag[$tempj]=0;}else{$egFlag[$tempi]=0;}}}}}open(efOut,">$egFileOut")or die"Error when saving data!\n";
